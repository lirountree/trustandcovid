...
school <- read.csv("Confounders Data/average-years-of-schooling/average-years-of-schooling.csv") %>% 
...
urban <- read.csv("Confounders Data/share-of-population-urban/share-of-population-urban.csv") %>% 
...
govt <- read.csv("Confounders Data/freedom.csv", skip=1) %>%  
...
write.csv(new_dat, "Regression dataframe.csv", row.names=FALSE)
